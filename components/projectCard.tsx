...
export default function ProjectCard({
  ...
  return (
    <div
      ref={cardRef}
      className="relative rounded-[60px] [clip-path:inset(0_round_60px)] cursor-pointer max-sm:rounded-[20px] max-sm:[clip-path:inset(0_round_20px)]"
    >
      <img
        ref={imgRef}
      ...
      <div
        ref={overlayRef}
        className="absolute inset-0 flex flex-col justify-end p-6 pb-8 max-sm:p-3 max-sm:pb-3 max-sm:bg-gradient-to-t max-sm:from-black/80 max-sm:to-transparent"
        style={{
      ...
      >
        <h3 className="font-panchang-bold text-4xl text-white max-sm:text-xl">{title}</h3>
        <div className="flex items-end justify-between mt-4 max-sm:mt-2">
          ...
          <div className="flex items-center gap-3 max-sm:gap-3">
            <a href={githubUrl} className="text-white/60 hover:text-white transition-colors">
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 max-sm:w-6 max-sm:h-6">
            ...
            <a href={previewUrl} className="text-white/60 hover:text-white transition-colors">
              <ArrowUpRight className="w-5 h-5 max-sm:w-6 max-sm:h-6" />
            </a>
          </div>
        </div>
      </div>
